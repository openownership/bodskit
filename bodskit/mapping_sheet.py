...
class MappingSheet:
    ...
    def handle(self):
        with open(self.input_filename) as fp:
            release = json.load(fp, object_pairs_hook=OrderedDict)

        release = JsonRef.replace_refs(release, base_uri=pathlib.Path(os.path.realpath(self.input_filename)).as_uri())
